...
```{r S13, eval=TRUE, include=TRUE}
group_list = c("GfIn0v2", "IeIn0v2", "GfIn2v0", "IeIn2v0", "GfIn1v1", "IeIn1v1")
A50 = paste0("Nip", group_list)
IR24 = paste0("Ir24", group_list)
...
for (m in c("RootLength","FreshWeight","PlantHeight")){
# m="RootLength"
# read raw data"RootLength"
df = read.table("SF12.txt", header=T, row.names=1, sep="\t")
...
index=subset(index, group %in% A50)
index$group = factor(index$group, levels = A50)
# boxplot stat code
model = aov(index[[m]] ~ group, data=index)
Tukey_HSD = TukeyHSD(model, ordered = TRUE, conf.level = 0.95)
Tukey_HSD_table = as.data.frame(Tukey_HSD$group) 
...
```{r S13, eval=TRUE, include=TRUE}
group_list = c("GfIn", "IeIn", "JeIn", "IehIn", "JehIn")
A50 = paste0("A50", group_list)
IR24 = paste0("Ir24", group_list)
...
# m="RootLength"
# read raw data"RootLength"
df = read.table("SF13.txt", header=T, row.names=1, sep="\t")
