---
title: "Rice Microbiome Fig. 2 Random forest model detects bacterial taxa that accurately predict indica and japonica subspeciation."
author: "Yong-Xin Liu"
date: "2018/6/12"
output: html_document
---

## Fig. 2A. Top Features to distinguish subspecies in family level.

(A) The top 18 bacterial families were identified by applying random forest classification of the relative abundance of the root microbiome in field II against indica and japonica rice. Biomarker taxa are ranked in descending order of importance to the accuracy of the model. The inset represents 10-fold cross-validation error as a function of the number of input families used to differentiate indica and japonica root microbiome in order of variable importance. The use of 18 families was sufficient to differentiate indica and japonica with an accuracy of 83.7%.
...
system("mkdir -p 2model")
```

### Select Field II (Training Set) data

Source ref in : script/randomForest_family.R

```{r reads_data, echo=TRUE}
# 一、Field II as train set, Field I as test sets 
...
otufile = "/mnt/bai/yongxin/rice/xianGeng/result/tax/sum_f.txt"
...
### Randomforest classification apply to training sets
...
set.seed(315)
otutab_t.rf= randomForest(group ~ ., data=otutab_t, importance=TRUE, proximity=TRUE) # ntree = 1000, nPerm = 10, mtry = 3, 
print(otutab_t.rf)
# MeanDecreaseAccuracy
varImpPlot(otutab_t.rf, main = "Top feature importance", n.var = 23) # , bg = par("bg"),color = par("fg"), gcolor = par("fg"), lcolor = "gray" 
write.table(otutab_t.rf$confusion, file = "2model/family_confusion.txt", sep = "\t", quote = F, row.names = T, col.names = T)
imp = as.data.frame(round(importance(otutab_t.rf), 2))
imp=imp[order(imp$MeanDecreaseAccuracy,decreasing = F),]
write.table(imp, file = "2model/family_imp.txt", sep = "\t", quote = F, row.names = T, col.names = T)
system("awk 'NR==FNR{a[$6]=$3} NR>FNR{print $0\"\t\"a[$1]}' ../result/taxonomy_8.txt 2model/family_imp.txt | sed '1 s/$/Phylum/' > 2model/family_imp_phylum.txt")
```


### Cross validation to choose porper number of features

Five time cross validation
...
write.table(allerr, file = "2model/family_rfcv.txt", sep = "\t", quote = F, row.names = T, col.names = T)
...
#  geom_hline(yintercept = min(allerr$err.mean), colour='black', lwd=0.36, linetype="dashed") + 
  ...
  main_theme
ggsave(p, file = "2model/family_rfcv.pdf", width = 89, height = 50, unit = 'mm')
p
```


### 2a1 Features barplot with taxonomy in phylum


```{r feature_imp, echo=TRUE}
imp = read.table("2model/family_imp_phylum.txt", header=T, row.names= 1, sep="\t") 
...
ggsave(paste("2model/family_top_feautre",".pdf", sep=""), p, width=89 * 1.5, height=50 * 1.5, unit='mm')
p
```

## Fig. 2B. Feature abundance barplot
...
otu_bar = sub_otutab[rownames(imp),] # 筛选最优的feature
mean = data.frame(id = rownames(otu_bar), mean=rowMeans(otu_bar))
mean = arrange(mean, desc(mean)) # 按均值排序 
...
p
ggsave(paste("2model/family_errorbar",".pdf", sep=""), p, width=89 * 1.5, height=50 * 2, unit='mm')

DA_list = read.table("/mnt/bai/yongxin/rice/xianGeng/xiangeng0wilcox/result/compare_f/HTEJ-HIND_all.txt", header=T, row.names=1, sep="\t") # , comment.char=""
DA_list = DA_list[rownames(imp) ,1:5]
# DA_list = DA_list[order(DA_list$logCPM, decreasing = T),]
...
### Plot japonica (TEJ) enriched family
...
p=p+theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1))
p
ggsave(paste("2model/Family_barplot_TEJ",".pdf", sep=""), p, width=89 * 0.6, height=50 * 1.5, unit='mm')
p
```

### Plot indica (IND) enriched family
...
p = ggplot(otu_error_bar[otu_error_bar$Family %in% IND, ], aes(x=Family, y=value, fill=subspecies)) + 
  ...
  geom_errorbar(aes(ymin=value-ci, ymax=value+ci),
                ...
                position=position_dodge(.9)) + main_theme
p=p+theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1))
ggsave(paste("2model/Family_barplot_IND",".pdf", sep=""), p,  width=89 * 1.5, height=50 * 1.5, unit='mm')
p
```

## Fig. 2C Prediction result of random forest model on Field I

(C) The results of prediction of indica and japonica rice in field I according to the random forest model. Each square represents an individual variety. The characteristics of the varieties are shown on the left. The predicted characteristics are shown on the right. Indica varieties (94.5% accuracy) are shown in red; japonica varieties (77.5% accuracy) are shown in blue. 
...
if (TRUE){
  ...
  #  Set group order
  sub_design$group  = factor(sub_design$group, levels=c("LTEJ","LIND")) # ,"LTEJ","LIND"
}
idx = rownames(sub_design) %in% colnames(otutab)
sub_design = sub_design[idx,]
sub_otutab = otutab[,rownames(sub_design)]
...
write.table(predict, file = "ST/04RF_prediction_FieldILN.txt", quote = F, row.names = T, col.names = T, sep = "\t")
system("sed -i 's/group/ID\tsubspecies/;s/IND/indica/g;s/TEJ/japonica/g' ST/04RF_prediction_FieldILN.txt")
...
pheatmap(matrix, color = c("#F9766E", "#00BFC4") , cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12,
         filename = "2model/family_test_IND.pdf")
...
pheatmap(matrix, color = c("#F9766E", "#00BFC4") , cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12,
         filename = "2model/family_test_TEJ.pdf")
pheatmap(matrix, color = c("#F9766E", "#00BFC4") , cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12)
```


## Fig. 2D. Classified varieties not in trainning set and grown in new locations

(D) The results of prediction of indica and japonica varieties that were not included in the training set and were grown in different geographical locations, according to the random forest model. The plants were grown in Changping Farm in northern China, which is located 2,482 km away from the fields in Lingshui Farm where the indica and japonica training sets were grown (Table S4). The results suggest that our model represents the general characteristics of the root microbiome from indica and japonica varieties grown in different geographical locations.  
...
write.table(matrix, file = "ST/04RF_predict_test2.txt", quote = F, row.names = T, col.names = T, sep = "\t")
system("sed -i 's/rep1/ID\trep1/;s/\t1/\tindica/g;s/\t2/\tjaponica/g' ST/04RF_predict_test2.txt")


# pheatmap(matrix, color = c("#F9766E", "#00BFC4") , cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12)
pheatmap(matrix[1:4, 1:16], color = c("#F9766E", "#00BFC4") , cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12,
         filename = "2model/family_test_nrt_IND.pdf")
pheatmap(matrix[1:4, 1:16], color = c("#F9766E", "#00BFC4") , cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12)

pheatmap(matrix[5:8,], color = c("#F9766E", "#00BFC4") , cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12,
         filename = "2model/family_test_nrt_TEJ.pdf")
pheatmap(matrix[5:8,], color = c("#F9766E", "#00BFC4") , cluster_rows = F, cluster_cols = F, cellwidth = 15, cellheight = 12)
```
